...
### More About Create Operations

###### Ordered Inserts

By default, if you try to write a few items into the database with insertMany, MongoDB will try to write it in the order it was written:
...
MongoDB will write the items in the order they were listed (ordered inserts). Since the first document and second document has the same id, it will cause an error and block the last document from being written. However, the operation doesn't rollback - the first document will be written. 

We can change the default behavior of our write operation by using configs. insertMany() takes a second argument as configs:
